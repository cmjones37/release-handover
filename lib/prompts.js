...
function buildPrdBlock(prdTexts) {
  if (prdTexts.length === 1) {
    return `PRD:\n${prdTexts[0].content}`;
  }
  const intro = `You are reviewing ${prdTexts.length} PRDs. Synthesise across all of them into a single, coherent output — do not produce separate sections per PRD.`;
  const sections = prdTexts.map((p, i) => `--- PRD ${i + 1}: ${p.name} ---\n\n${p.content}`).join('\n\n');
  return `${intro}\n\nPRDs:\n\n${sections}`;
}

const PROMPTS = {

  customer: (prdTexts) => `You are generating a customer-facing release note from a Product Requirements Document (PRD).
...
${buildPrdBlock(prdTexts)}`,

  gtm: (prdTexts) => `You are generating a GTM (Go-to-Market) briefing for sales and marketing from a Product Requirements Document (PRD).
...
${buildPrdBlock(prdTexts)}`,
  ...
  services: (prdTexts) => `You are generating a services and support briefing from a Product Requirements Document (PRD).
...
${buildPrdBlock(prdTexts)}`,
  ...
  leadership: (prdTexts) => `You are generating a leadership briefing from a Product Requirements Document (PRD).
